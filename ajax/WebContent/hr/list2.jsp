...
<script type="text/javascript">
	function openPopup(event) {
		// 링크 클릭시 페이지가 이동되는 기본동작이 발생하지 않도록 방해
		event.preventDefault();
		// 사원 정보를 표시할 팝업을 화면에 나타냄
		document.getElementById('popup').style.display = "block";
		
		// event.target에는 지금 클릭한 <a> 태그의 엘리먼트 객체가 들어있음
		// 해당 엘리먼트에서 href 속성값을 조회 <--- "data2.jsp?id=사원아이디" 정보
		...
		// onreadystatechange는 XHR의 readyState 값이 변할 때마다 발생하는 이벤트
		xhr.onreadystatechange = function() {
			// 콜백함수가 4번 실행되기는 하지만, 실질적인 작업이 구현된 아래의 소스는
			// 하나의 XHR에 대해서 단 한번만 실행됨
			if(xhr.readyState == 4 && xhr.status == 200) {
				// 서버가 보낸 응답데이터 조회 <--- XHR의 responseText는 응답데이터를 보관하고 있음
				var text = xhr.responseText;
				// 응답데이터가 JSON 텍스트면 JSON.parse(json표기법의 텍스트) 메서드를 실행
				// 텍스트 데이터를 자바스크립트 배열이나 객체로 변환
	...
	function getEmployees() {
			...
			// XHR 객체에서 응답 데이터 가져오기
			// XHR 객체의 readyState는 요청처리의 각 과정에 따라서 0 ~ 4로 값 변경됨
			// 			  readyState 값이 변할 때마다 onreadystatechange 이벤트 발생
			// 			  readyState 값이 4면 응답 받은 것
			// 			  state 값이 200이면 성공적인 응답 받은 것
			// 서버가 보낸 응답 데이터는 XHR 객체의 responseText에 보관됨
			// 서버가 보낸 응답 데이터가 JSON 형식의 데이터라면 JSON.parse() 함수를 사용해서
			//	javascript의 배열이나 객체로 변환 가능
			// 배열이나 객체로 변환되었다면 자바스크립트 문법을 사용해서 값을 조회,
			// 조회된 값을 이용해서 HTML 컨텐츠를 생성, 특정 태그에 부착하여 화면에 표시함
			xhr.onreadystatechange = function() {
				if(xhr.readyState == 4 && xhr.status == 200) {
					var text = xhr.responseText;
