								...
								<div class="card">
									...
									<div class="card-body">
										...
										<div class="d-flex justify-content-between">
											<span class="text-secondary font-weight-bold">${todo.userId }</span>											
     					...
     					<div class="col-12">
     						<table class="table table-bordered table-sm">
     							...
     							<tbody>
     								<tr>
     									<th>번호</th>
     									<!-- <input type="hidden" id="modal-to-no" value=""/> -->
	...
	function openTodoDetailModal(todoNo) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				// document.getElementById("modal-todo-no").value= resData.no;
				
				if (resData.CanModify == true) {
				...
				document.getElementById("modal-todo-username").textContent = resData.userId;
		...
		//xhr.open("post", "/todo/detail.hta");
		//xhr.setRequestHeader('Content-type', "application/x-www-form-urlencoded");
		//xhr.send("todoNo=" + todoNo);
	...
	function updateTodoStatus(status) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				// 상세정보의 상태, 뱃지를 바꾼다 statusClass name을 바꿈
				//id="card-todo-status-${todo.no }
				var change = document.getElementById("card-todo-status-" + resData.no);
				// 페이지 리로드 하지않고 뱃지 바뀌게 해놓기
