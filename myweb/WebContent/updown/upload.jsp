...
<%
	...
	// MultipartRequest 객체
	// * cos.jar 라이브러리에서 첨부파일 업로드를 지원하는 클래스
	// * 첨부파일이 저장될 디렉토리, 최대 업로드 사이즈, 문자인코딩 방식을 지정하면
	//   객체 생성시 첨부파일이 지정된 디렉토리에 자동으로 저장됨
	// * 요청 파라미터값을 조회하는 기능과 파일이름을 조회하는 기능을 제공
	// * 요청 파라미터값을 조회하는 기능
	...
	// 	동일한 파일 이름이 존재하는 경우 파일 이름을 변경해주는 객체
	//	ex)	logo.jpg가 존재할 때 이와 동일한 이름의 파일이 업로드 되면 logo1.jpg로 저장
	
	MultipartRequest mr = new MultipartRequest(request, saveDirectory, maxPostSize, encoding, new DefaultFileRenamePolicy());
	
	// 리퀘스트 객체로 받을 수 없음 (null값 뜸)
